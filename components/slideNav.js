    ...
    },
    {
      ...
      value: function observe() {
        ...
        window.addEventListener("click", function(e) {
          if (
            _this.opened &&
            !_this.isClosestElement(e.target, _this.toggleButton) &&
            !_this.isBoxNavTarget(e.target)
          ) {
            _this.hideNavBox();
          }
        });
      ...
      value: function setActiveAnchor() {
        ...
        try {
          for (
            ...
            if (
              section &&
              ((section.offsetTop <= Math.round(offset) &&
                section.offsetTop + section.offsetHeight >
                  Math.round(offset)) ||
                Math.ceil((offset + window.innerHeight) / 100) * 100 ==
                  Math.ceil(scrollHeight / 100) * 100)
            ) {
